...
public class GlobalHttpHandlerImpl implements GlobalHttpHandler {
    ...
    public Response onHttpResultResponse(String httpResult, Interceptor.Chain chain, Response response) {
        ...
        if (!TextUtils.isEmpty(httpResult) && RequestInterceptor.isJson(response.body().contentType())) {
            try {
                ResponseBaseEntity<Did> responseBaseEntity = ArmsUtils.obtainAppComponentFromContext(context).gson().fromJson(httpResult, new TypeToken<ResponseBaseEntity<Did>>() {
                }.getType());
                Timber.w("Result ------> " + responseBaseEntity.getMsg());
                Log.e("wuzhiyun", responseBaseEntity.getMsg() + " " + responseBaseEntity.getData().getDid());
            } catch (Exception e) {
                e.printStackTrace();
                return response;
            }
